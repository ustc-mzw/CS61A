...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if opponent_score-3 < player_score < opponent_score:
        return True
    else:
        return False
...
# def play_a_turn(who, player_score, opponent_score):
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who == 0:
            num_dices = strategy0(score0, score1)
            score0 += take_turn(num_dices, score1, dice)
            if not extra_turn(score0, score1):
                who = other(who)
        else:
            num_dices = strategy1(score1, score0)
            score1 += take_turn(num_dices, score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_highest(score0, score1):
        if who == 0:
            ...
            score = score1
        cur_increment = score-last_score
        max_increment = running_high
        if cur_increment>max_increment:
            print(cur_increment,"point(s)! The most yet for Player",who)
            max_increment = cur_increment
        return  announce_highest(who, score, max_increment)
    return say_highest
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average_function(*args):
        ...
        cnt = trials_count
        while cnt > 0:
            result_sum += original_function(*args)
            cnt -= 1
        return result_sum/trials_count
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1,11):
        cur_score = average_roll_dice(i, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if free_bacon_score >= cutoff or extra_turn(score+free_bacon_score, opponent_score):
        return 0
    else:
        return num_rolls 
    # END PROBLEM 11
